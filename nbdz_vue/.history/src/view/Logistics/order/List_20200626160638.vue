      ...
      <Col :span="24">
        <Form ref="formInline" label-position="right" :label-width="100" inline>
          ...
          <FormItem label="订单支付开始时间">
            <Input clearable v-model="filters.startTime" placeholder="请输入搜索内容"></Input>
          </FormItem>
          <FormItem label="订单支付结束时间">
            <Input clearable v-model="filters.endTime" placeholder="请输入搜索内容"></Input>
          </FormItem>
          <FormItem>
...
<script>
import { order as getList } from "@/api/Logistics";
  ...
  methods: {
    loadData() {
      ...
      const data = {
        ...
        paid_data_gte: _this.filters.startTime,
        paid_data_lte: _this.filters.endTime
      };
      getList(data)
        .then(res => {
