...
export default {
  ...
  methods: {
    loadData() {
      ...
      if (deliveredTime) {
        data.DeliveredTime_gte = paidTime.DeliveredTime_gte;
        data.DeliveredTime_lte = paidTime.DeliveredTime_lte;
      }
      getList(data)
        .then((res) => {
    ...
    filtersDate(stringStart, stringEnd, startTime, endTime) {
      ...
      if (startTime != "" && endTime != "") {
        ...
        if (!dayjs(endTime).isAfter(dayjs(startTime))) {
          ...
          let data = {};
          data.stringStart = dayjs(_this.filters.startTime).format(
            "YYYY-MM-DD"
          );
          data.stringEnd = dayjs(_this.filters.endTime).format("YYYY-MM-DD");
          return data;
        }
      }
    },
