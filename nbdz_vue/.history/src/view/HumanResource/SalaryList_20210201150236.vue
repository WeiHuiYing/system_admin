    ...
    <Row style="margin-bottom: 10px">
      <Col :span="22">
        <Form ref="formInline" label-position="right" :label-width="90" inline>
          ...
          <FormItem label="职位">
            <Input style="width: 150px" clearable v-model="filters.Position" />
          </FormItem>
...
import { fileToJson } from "@/lib/validator";
...
export default {
  ...
  methods: {
    loadData(type) {
      ...
      if (_this.filters.Date && _this.filters.Date != "") {
        _this.filters.Date = dayjs(_this.filters.Date).format("YYYY-MM-DD");
      }
    ...
    importExcel(file) {
      ...
      importSalary(formData)
        ...
        .catch((err) => {
          ...
          const response = err.response;
          if (response) {
            fileToJson(response.data).then((errData) => {
              if (errData && errData.detail) {
                this.$Message.error({
                  content: errData.detail,
                  duration: 10,
                  closable: true,
                });
              } else if (errData) {
