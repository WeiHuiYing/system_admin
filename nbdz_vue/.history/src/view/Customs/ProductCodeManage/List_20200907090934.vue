...
<template>
  <div class="content-main">
    ...
    <Table
      ...
      stripe
    >
      <template slot-scope="{ row,rowIndex }" slot="HsCode">
        <Select
          @on-change="HsCodeChange(row,rowIndex)"
          v-if="row.isEdit"
          v-model="row.HsCode"
          filterable
          remote
...
export default {
  ...
  methods: {
    ...
    HsCodeChange(row,rowIndex){
      console.log(row)
      console.log(rowIndex)
      let codeItem = this.HsCodeList.filter(item => {if(item.HsCode ==row.HsCode){ return item} });
      ...
      this.$set(this.HsCodeList,row,rowItem)
    }
  },
  mounted() {
    this.loadData();
