      ...
      <Col :span="4">
        <Dropdown v-if="selectionList.length > 0" placement="bottom-start" @on-click="handleSelect">
          ...
          <DropdownMenu slot="list">
            <DropdownItem name="Order">生成批次订单</DropdownItem>
            <DropdownItem name="Batch">生成E贸易订单</DropdownItem>
          </DropdownMenu>
        </Dropdown>
      </Col>
    </Row>
...
export default {
  ...
  methods: {
    ...
    createOrder(name) {
      ...
      if (name == "Order") {
        data = {
          id: _this.selectionList.join(","),
        };
        request = getList(data, "PUT");
      } else if (name == "Batch") {
        data = {
          batch_ids: _this.selectionList.join(","),
        };
        request = createEorder(data, "POST");
      }
      _this.tableLoading = true;
