...
export class Storage {
  ...
  setItem(params) {
    ...
    if (options.expires) {
      ...
      let type = Object.prototype.toString.call(options.val);
      //如果value是对象或者数组对象的类型，就先用JSON.stringify转一下，再存进去
      if (Object.prototype.toString.call(options.val) == '[object Object]') {
        options.val = JSON.stringify(options.val);
      }
      if (Object.prototype.toString.call(options.val) == '[object Array]') {
        options.val = JSON.stringify(options.val);
      }
      localStorage.setItem(options.key, options.val);
    }
  }
  //拿到缓存
  getItem(key) {
