...
class HttpRequest {
  constructor(baseApi = baseApi, baseLog = baseLog, AuthApi = AuthApi, loginApi = loginApi, newApi = newApi) {
  ...
  getInsideConfig(options) {
    ...
    } else if (new RegExp("^/newApi/").test(options.url)) {
      config = {
        baseURL: this.newApi,
        headers: {
          //
  ...
  request(options) {
    ...
    options.url = options.url.replace("/newApi/", "/newApi/")
