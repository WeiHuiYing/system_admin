    ...
    <Row class="search-con search-con-top">
      <Col :span="22">
        <Form ref="formInline" label-position="right" :label-width="120" inline>
          ...
          <FormItem prop="startDeliveryDate" label="提货开始时间">
            <DatePicker
              v-model="filters.startDeliveryDate"
              type="date"
              :options="dateOptions"
              placeholder="请选择开始时间"
              style="width: 200px"
          ...
          <FormItem prop="endDeliveryDate" label="提货结束时间">
            <DatePicker
              v-model="filters.endDeliveryDate"
              type="date"
              :options="dateOptions"
              placeholder="请选择结束时间"
              style="width: 200px"
...
export default {
  ...
  methods: {
    loadData(type) {
      ...
      let filterDelivery = filtersDate(
        "startDeliveryDate",
        _this.filters.startDeliveryDate,
        _this.filters.endDeliveryDate,
        "endDeliveryDate"
      );
      if (filterDelivery) {
        if (filterDelivery.length > 0) {
          filtersquery = filtersquery.concat(filterDelivery);
        }
      }
      let data = {
    ...
    filtersObj() {
      ...
      Object.keys(_this.filters).forEach((keyItem) => {
        if (_this.filters[keyItem] && _this.filters[keyItem] != "") {
          if (keyItem != "startDeliveryDate" && keyItem != "endDeliveryDate") {
            filterQuery.push({
              key: keyItem,
              option: "lk",
